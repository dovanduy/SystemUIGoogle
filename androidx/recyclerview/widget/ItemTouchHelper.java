...
public class ItemTouchHelper extends ItemDecoration implements OnChildAttachStateChangeListener
{
    ...
    private int checkHorizontalSwipe(final ViewHolder viewHolder, final int n) {
        if ((n & 0xC) != 0x0) {
            ...
            if (mVelocityTracker != null && this.mActivePointerId > -1) {
                ...
                final float mMaxSwipeVelocity = this.mMaxSwipeVelocity;
                mCallback.getSwipeVelocityThreshold(mMaxSwipeVelocity);
                mVelocityTracker.computeCurrentVelocity(1000, mMaxSwipeVelocity);
                ...
                if ((n2 & n) != 0x0 && n3 == n2) {
                    ...
                    final float mSwipeEscapeVelocity = this.mSwipeEscapeVelocity;
                    mCallback2.getSwipeEscapeVelocity(mSwipeEscapeVelocity);
                    if (abs >= mSwipeEscapeVelocity && abs > Math.abs(yVelocity)) {
                        return n2;
                    }
                }
            }
        ...
        if ((n & 0x3) != 0x0) {
            ...
            if (mVelocityTracker != null && this.mActivePointerId > -1) {
                ...
                if ((n2 & n) != 0x0 && n2 == n3) {
                    ...
                    final float mSwipeEscapeVelocity = this.mSwipeEscapeVelocity;
                    mCallback2.getSwipeEscapeVelocity(mSwipeEscapeVelocity);
                    if (abs >= mSwipeEscapeVelocity && abs > Math.abs(xVelocity)) {
                        return n2;
                    }
                }
            }
    ...
    private void destroyCallbacks() {
        ...
        for (int i = this.mRecoverAnimations.size() - 1; i >= 0; --i) {
            final RecoverAnimation recoverAnimation = this.mRecoverAnimations.get(0);
    ...
    void moveIfNecessary(final ViewHolder viewHolder) {
        ...
        final int absoluteAdapterPosition = chooseDropTarget.getAbsoluteAdapterPosition();
        final int absoluteAdapterPosition2 = viewHolder.getAbsoluteAdapterPosition();
        if (this.mCallback.onMove(this.mRecyclerView, viewHolder, chooseDropTarget)) {
            this.mCallback.onMoved(this.mRecyclerView, viewHolder, absoluteAdapterPosition2, chooseDropTarget, absoluteAdapterPosition, n, n2);
        }
    }
        ...
        this.mRecyclerView.post((Runnable)new Runnable() {
            ...
            public void run() {
                ...
                if (mRecyclerView != null && mRecyclerView.isAttachedToWindow()) {
                    ...
                    if (!val$anim.mOverridden && val$anim.mViewHolder.getAbsoluteAdapterPosition() != -1) {
            ...
            if (mSelected2.itemView.getParent() != null) {
                ...
                if (swipeIfNecessary != 1 && swipeIfNecessary != 2) {
                    if (swipeIfNecessary != 4 && swipeIfNecessary != 8 && swipeIfNecessary != 16 && swipeIfNecessary != 32) {
                        ...
                        final float signum = Math.signum(this.mDx);
                        final float n3 = (float)this.mRecyclerView.getWidth();
                        n2 = 0.0f;
                        n = signum * n3;
                    }
                }
                else {
                    final float signum2 = Math.signum(this.mDy);
                    final float n4 = (float)this.mRecyclerView.getHeight();
                    n = 0.0f;
                    n2 = signum2 * n4;
                }
    ...
    public abstract static class Callback
    {
        ...
        public abstract boolean canDropOver(final RecyclerView p0, final ViewHolder p1, final ViewHolder p2);
    ...
    static class RecoverAnimation implements Animator$AnimatorListener
    {
        ...
        RecoverAnimation(final ViewHolder mViewHolder, final int n, final int mActionState, final float mStartDx, final float mStartDy, final float mTargetX, final float mTargetY) {
